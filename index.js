	...
	case 'text':
		...
		case '我的名字':
			event.source.profile().then(function (profile) {
				return event.reply('小看77我 ~ 我當然知道你叫「' + profile.displayName + '」阿!!');
			});
			break;	
		...
		case '蛤':
			return event.reply({
			"type": "image",
			"originalContentUrl": "https://imgs.niusnews.com/upload/imgs/default/2017NovC/1113cat/yout.png",
			"previewImageUrl": "https://imgs.niusnews.com/upload/imgs/default/2017NovC/1113cat/yout.png"
			});
			break;
		...
		//random 中餐選擇區	
		case '低價位':
			var items = Array("宮保王","鍋燒意麵","巷口第二家","天祥路炒飯","Sukiya家","麥當勞超值午餐","水煎包","早餐店","民權自助餐","池上便當");			
		...
		case '中價位':
			var items = Array("摩斯漢堡","長崎冠軍麵","圈子","頂呱呱","老地方牛肉麵","巧珍園","福勝亭","十巷咖哩");			
			var item = items[Math.floor(Math.random()*items.length)];
		...
		case '高價位':
			var items = Array("圈子(高價位款)","一丁壽喜燒","沙汀娜好食","大橋頭第一鍋物","好食多");			
			var item = items[Math.floor(Math.random()*items.length)];
